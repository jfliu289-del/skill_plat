...
- **Never embed credentials in destinations that point to third-party hosts.** Destination `headers` and `body_template` fields are stored server-side. Only include `MATON_API_KEY` when the destination URL is `https://api.maton.ai/` — never for any other host.
