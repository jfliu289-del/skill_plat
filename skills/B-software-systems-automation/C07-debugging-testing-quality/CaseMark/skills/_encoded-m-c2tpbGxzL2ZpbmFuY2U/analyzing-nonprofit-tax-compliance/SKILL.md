...
2. **Review Form 990 accuracy** — Cross-check reported figures against financial statements. Verify:
...
3. **Analyze UBIT exposure** — For each revenue stream, determine whether it constitutes an unrelated trade or business:
...
4. **Evaluate private foundation rules** (if applicable) — Assess compliance with:
...
5. **Assess governance and operational compliance** — Review:
...
6. **Review international and cross-border issues** — If the organization has foreign operations or grantmaking:
...
7. **Compile findings and risk assessment** — Categorize issues by severity (critical/moderate/low), assign remediation recommendations, and flag items requiring immediate corrective action.
