...
Always returns None (used to drop / mask a branch). It never raises, whether
`x` is present or None. The symbolic path keeps the schema for graph
construction.

```python
result = ~x  # None whether x is present or None
```
